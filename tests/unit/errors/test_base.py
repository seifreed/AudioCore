...
class TestAudioCoreError:
    ...
    def test_cause_parameter_stores_cause_without_from(self) -> None:
        """Regression: AudioCoreError(cause=...) should store cause for programmatic access.

        The cause= constructor parameter stores the original exception in
        original_cause, without conflicting with Python's `raise X from Y`
        mechanism that sets __cause__.
        """
        original = RuntimeError("disk I/O failure")
        error = AudioCoreError("Transcription failed", cause=original)
        assert error.original_cause is original
        assert isinstance(error.original_cause, RuntimeError)
        assert "disk I/O failure" in str(error.original_cause)
