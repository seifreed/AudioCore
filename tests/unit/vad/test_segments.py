...
class TestMergeShortSegments:
    ...
    def test_merge_single_orphan_short_segment_kept(self, default_config: VADConfig) -> None:
        ...
        Previously, merge_short_segments dropped a single short segment that
        couldn't be merged with any neighbor, producing empty output for
        legitimate short utterances. Now it is preserved.
        """
        segments = [(0.0, 0.1, 0.80)]  # 0.1s, well below default min 0.5s
        result = merge_short_segments(segments, default_config)
        assert len(result) == 1
        assert result[0] == (0.0, 0.1, 0.80)
